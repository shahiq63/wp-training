...
function us_ajax_cform() {
	...
	$content = $matches[4][ $form_index - 1 ];
	...
	if ( isset( $fields['name'] ) AND isset( $shortcode_atts['name_field'] ) AND $shortcode_atts['name_field'] != 'hidden' AND $_POST['name'] != '' ) {
		$body_name_part = '<strong>' . sanitize_text_field( stripslashes( $_POST['name'] ) ) . '</strong>';
	} else {
	...
	if ( isset( $fields['email'] ) AND isset( $shortcode_atts['email_field'] ) AND $shortcode_atts['email_field'] != 'hidden' AND $_POST['email'] != '' ) {
		if ( is_rtl() ) {
			$body_email_part = '<strong>' . sanitize_email( stripslashes( $_POST['email'] ) ) . '</strong> :' . us_translate( 'Email' );
		} else {
			$body_email_part = us_translate( 'Email' ) . ': <strong>' . sanitize_email( stripslashes( $_POST['email'] ) ) . '</strong>';
		}
	...
	if ( isset( $fields['phone'] ) AND isset( $shortcode_atts['phone_field'] ) AND $shortcode_atts['phone_field'] != 'hidden' AND $_POST['phone'] != '' ) {
		if ( is_rtl() ) {
			$body_phone_part = '<strong>' . sanitize_text_field( stripslashes( $_POST['phone'] ) ) . '</strong> :' . __( 'Phone Number', 'us' );
		} else {
			$body_phone_part = __( 'Phone Number', 'us' ) . ': <strong>' . sanitize_text_field( stripslashes( $_POST['phone'] ) ) . '</strong>';
		}
	} else {
		$body_phone_part = '';
	}
	...
	// Adding the details of the consent
	if ( isset( $fields['checkbox'] ) AND isset( $shortcode_atts['checkbox_field'] ) AND $shortcode_atts['checkbox_field'] != 'hidden' AND $_POST['checkbox'] != '' ) {
		$email_body .= '<p>' . __( 'Agreement text', 'us' ) . ': ' . strip_tags( $content, '<a><strong>' ) . '<br>';
		$email_body .= __( 'Agreement date and time', 'us' ) . ': ' . gmdate( 'Y-m-d H:i:s' ) . ' GMT<br>';
		$email_body .= __( 'IP address', 'us' ) . ': ' . us_get_ip() . '</p>';
	}
